...
double ScanMatcher::optimize(OrientedPoint& pnew, const ScanMatcherMap& map, const OrientedPoint& init, const double* readings) const
{
	...
	//score函数求当前机器人位姿p，在当前地图的匹配得分，越匹配，位姿得分越高，说明位姿越准确
	...
	//就这样在运动模型的更新的机器人位姿基础之上，不断的超不同的方向，一层一层的向上爬，同时进行位姿修改，直到位姿得分出现下降，或者迭代结束
    do
    {
		...
		//初始化bestScore，前面在运动模型的更新的机器人位姿基础上计算的得分
...
{
	/*把激光雷达的位置转换到地图坐标系*/
	//lp地图坐标系下的机器人位姿，也为激光雷达坐标系的位姿，程序默认激光雷达传感器和机器人坐标系重合
	OrientedPoint lp=p;
...
void ScanMatcher::registerScan(ScanMatcherMap& map, const OrientedPoint& p, const double* readings)
{
	...
	/*把激光雷达的位置转换到地图坐标系，默认base和laser重合*/
	OrientedPoint lp=p;
