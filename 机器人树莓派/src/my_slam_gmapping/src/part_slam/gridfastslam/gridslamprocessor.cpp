...
namespace GMapping {
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles)
  {
    ...
    last_update_time_+=1.0;
	  ...
	  /*只有当机器人走过一定的距离 或者 旋转过一定的角度 或者 过一段指定的时间才处理激光数据 或者 处理第一帧数据*/
    if (!m_count 
      ...
      if (m_count>0)
      {
        /*功能：为每一个粒子，在当前机器人位姿下，当前帧激光数据下，求解每个粒子的位姿最优值（策略：爬山，评价机制：得分机制（当前位姿下激光数据和当前地图的的匹配程度））
                求解每个粒子的权重
