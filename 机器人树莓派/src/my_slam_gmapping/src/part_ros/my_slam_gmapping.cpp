...
void MySlamGMapping::startLiveSlam()
{
...

    {
        ...
        //tf::MessageFilter，将scan转换到odom坐标系，变换关系存放到tf_，在getOdomPose函数中使用，找到当前scan时的odom_pose
...
void MySlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    GMapping::OrientedPoint odom_pose;         //当前里程计坐标系下的机器人位姿的临时变量    
    if(addScan(*scan, odom_pose))
    {
        ...
        //在机器人当前位姿下，用当前的里程计坐标系下的机器人位姿和map坐标系下的机器人位姿，来表述里程计坐标系和map坐标系的差距
        //因为两个坐标系描述的是同一个机器人，所以才能用位姿的不同描述坐标系变换关系（差异）
        ...
        map_to_odom_ = map_to_base * (odom_to_base.inverse()); //表述里程计坐标系和map坐标系的差距 
...
//通过每一帧scan的时间戳，计算出当前里程计坐标系下机器人的位姿
bool MySlamGMapping::getOdomPose(GMapping::OrientedPoint& gmap_pose, const ros::Time& t)
{
    ...
    //odom_pose存储输出的里程计坐标系下机器人tf::Stamped<tf::Pose>格式的位姿
